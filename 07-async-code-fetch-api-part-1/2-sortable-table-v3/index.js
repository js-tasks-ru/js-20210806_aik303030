...
export default class SortableTable {
  ...
  downloadListener = async () => {
    if (this.subElements.body.getBoundingClientRect().bottom < document.documentElement.clientHeight + 200 && !this.loading && !this.isSortLocally) {
      this.loading = true;
      const data = await this.loadData(this.latestId, this.latestOrder, this.offset, this.offset + this.limit);
  ...
  render() {
    ...
    this.loadData(this.latestId, this.latestOrder, this.start, this.start + this.limit)
      .then((data) => {
  ...
  async sortOnServer (id, order) {
    this.data = await this.loadData(id, order, this.start, this.offset);
    this.updateTable(this.data);
  }
